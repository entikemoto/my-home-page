...
- **`/less-permission-prompts` スキル**（v2.1.111）: 即適用。allowlist を手動分析して 8 件追加した（詳細は下記）。
- **`/ultrareview`**（v2.1.111）: 検討。開発プロジェクトのレビュー時に試す候補。
- **Opus 4.7 + `xhigh` effort**（v2.1.111）: 確認のみ。**このアカウントでは未開放**（2026-04-17 時点で `/model` ピッカーに 4.6 のみ表示）のため、段階的ロールアウト待ち。
- **Auto mode 自動有効化**（v2.1.111）: 不要。既存運用への影響なし。
- **読み取り専用 glob コマンドの権限スキップ**（v2.1.111）: 不要。Claude Code が自動処理。
- **"Auto (match terminal)" テーマ**（v2.1.111）: 不要。個人設定の範囲。
