...
## 実行環境ルーティング（コマンド実行前に必ず確認）

Bash コマンドを実行する前に、以下の表で環境を判定すること。
**迷ったら SBX を選ぶ。**

| 操作カテゴリ | 環境 | 理由 |
|------------|------|------|
| ファイル編集（Edit / Write / Read） | **ローカル** | git で追跡可能・可逆 |
| `npm run dev / build / test / lint` | **ローカル** | プロセス内で完結 |
| `npx tsc --noEmit` | **ローカル** | 読み取り専用 |
| `git add / commit / push` | **ローカル** | 標準ワークフロー |
| `npm install <パッケージ>` | **SBX** | ホスト環境汚染リスク |
| 外部 API / Webhook への書き込み | **SBX** | 副作用が外部に出る |
| `rm / rmdir` 系・大量削除 | **SBX** | 不可逆 |
| 未知スクリプト・curl pipe bash 等 | **SBX** | Trojan Source リスク |

### SBX コマンドの実行形式

```bash
docker run --rm -v "$(pwd)":/app -w /app node:24 sh -c "<コマンド>"
```

### 判定フロー（迷ったときの一問一答）

1. このコマンドは外部ネットワークに **書き込む** か？ → **SBX**
2. このコマンドは **元に戻せない** 変更をするか？ → **SBX**
3. このコマンドは **未知・未検証のコード** を実行するか？ → **SBX**
4. 上記すべて No → **ローカル**
